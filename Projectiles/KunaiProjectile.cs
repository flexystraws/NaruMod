...
namespace NaruMod.Projectiles
{
	public class KunaiProjectile : ModProjectile
	{
		...
		public override void Kill(int timeLeft) {
			...
			if (projectile.owner == Main.myPlayer) {
				...
				Main.rand.NextBool(18)
					? Item.NewItem(projectile.getRect(), ItemType<Kunai>())
					: 0;
		...
		public override void ModifyHitNPC(NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection) {
			...
			//target.AddBuff(BuffType<Buffs.K>(), 900); // Adds the ExampleJavelin debuff for a very small DoT
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
				    ...
				    && currentProjectile.modProjectile is KunaiProjectile javelinProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
